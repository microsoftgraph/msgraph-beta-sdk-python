...
class OwnersWithUserPrincipalNameRequestBuilder(BaseRequestBuilder):
    """
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]], user_principal_name: Optional[str] = None) -> None:
        ...
        super().__init__(request_adapter, "{+baseurl}/teamTemplateDefinition/{teamTemplateDefinition%2Did}/teamDefinition/owners(userPrincipalName='{userPrincipalName}'){?%24expand,%24select}", path_parameters)
    
    async def get(self,request_configuration: Optional[RequestConfiguration[OwnersWithUserPrincipalNameRequestBuilderGetQueryParameters]] = None) -> Optional[User]:
        """
    ...
    def to_get_request_information(self,request_configuration: Optional[RequestConfiguration[OwnersWithUserPrincipalNameRequestBuilderGetQueryParameters]] = None) -> RequestInformation:
        """
        The list of this team's owners. Currently, when creating a team using application permissions, exactly one owner must be specified. When using user-delegated permissions, no owner can be specified (the current user is the owner). The owner must be specified as an object ID (GUID), not a UPN.
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
    ...
    def with_url(self,raw_url: str) -> OwnersWithUserPrincipalNameRequestBuilder:
        """
        ...
        return OwnersWithUserPrincipalNameRequestBuilder(self.request_adapter, raw_url)
    ...
    @dataclass
    class OwnersWithUserPrincipalNameRequestBuilderGetRequestConfiguration(RequestConfiguration[OwnersWithUserPrincipalNameRequestBuilderGetQueryParameters]):
        """
