...
class ContentRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, Dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/users/{user%2Did}/messages/{message%2Did}/$value{?%24format*}", path_parameters)
    ...
    def to_put_request_information(self,body: bytes, request_configuration: Optional[ContentRequestBuilderPutRequestConfiguration] = None) -> RequestInformation:
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.PUT, '{+baseurl}/users/{user%2Did}/messages/{message%2Did}/$value', self.path_parameters)
        request_info.configure(request_configuration)
