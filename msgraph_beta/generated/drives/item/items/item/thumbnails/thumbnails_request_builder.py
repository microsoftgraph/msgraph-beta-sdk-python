...
class ThumbnailsRequestBuilder(BaseRequestBuilder):
    ...
    async def get(self,request_configuration: Optional[RequestConfiguration] = None) -> Optional[ThumbnailSetCollectionResponse]:
        """
    ...
    async def post(self,body: Optional[ThumbnailSet] = None, request_configuration: Optional[RequestConfiguration] = None) -> Optional[ThumbnailSet]:
        """
    ...
    def to_post_request_information(self,body: Optional[ThumbnailSet] = None, request_configuration: Optional[RequestConfiguration] = None) -> RequestInformation:
        """
        Create new navigation property to thumbnails for drives
        param body: The request body
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.POST, '{+baseurl}/drives/{drive%2Did}/items/{driveItem%2Did}/thumbnails', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
