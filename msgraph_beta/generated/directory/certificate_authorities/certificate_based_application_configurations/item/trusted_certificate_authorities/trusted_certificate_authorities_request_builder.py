...
class TrustedCertificateAuthoritiesRequestBuilder(BaseRequestBuilder):
    ...
    def to_post_request_information(self,body: Optional[CertificateAuthorityAsEntity] = None, request_configuration: Optional[TrustedCertificateAuthoritiesRequestBuilderPostRequestConfiguration] = None) -> RequestInformation:
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.POST, '{+baseurl}/directory/certificateAuthorities/certificateBasedApplicationConfigurations/{certificateBasedApplicationConfiguration%2Did}/trustedCertificateAuthorities', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
