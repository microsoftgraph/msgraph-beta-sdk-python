...
if TYPE_CHECKING:
    from ....models.conditional_access_template_collection_response import ConditionalAccessTemplateCollectionResponse
    from ....models.o_data_errors.o_data_error import ODataError
    from .count.count_request_builder import CountRequestBuilder
    from .item.conditional_access_template_item_request_builder import ConditionalAccessTemplateItemRequestBuilder
...
class TemplatesRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, Dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/identity/conditionalAccess/templates{?%24count,%24expand,%24filter,%24orderby,%24search,%24select,%24skip,%24top}", path_parameters)
    
    def by_conditional_access_template_id(self,conditional_access_template_id: str) -> ConditionalAccessTemplateItemRequestBuilder:
        """
        Provides operations to manage the templates property of the microsoft.graph.conditionalAccessRoot entity.
        param conditional_access_template_id: The unique identifier of conditionalAccessTemplate
        Returns: ConditionalAccessTemplateItemRequestBuilder
        """
        if conditional_access_template_id is None:
            raise TypeError("conditional_access_template_id cannot be null.")
        from .item.conditional_access_template_item_request_builder import ConditionalAccessTemplateItemRequestBuilder

        url_tpl_params = get_path_parameters(self.path_parameters)
        url_tpl_params["conditionalAccessTemplate%2Did"] = conditional_access_template_id
        return ConditionalAccessTemplateItemRequestBuilder(self.request_adapter, url_tpl_params)
    
    async def get(self,request_configuration: Optional[RequestConfiguration[TemplatesRequestBuilderGetQueryParameters]] = None) -> Optional[ConditionalAccessTemplateCollectionResponse]:
        """
