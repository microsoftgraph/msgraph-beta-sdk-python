...
class FederatedIdentityCredentialsWithNameRequestBuilder(BaseRequestBuilder):
    ...
    def to_get_request_information(self,request_configuration: Optional[RequestConfiguration[FederatedIdentityCredentialsWithNameRequestBuilderGetQueryParameters]] = None) -> RequestInformation:
        ...
        Returns: RequestInformation
        """
        request_info = RequestInformation(Method.GET, '{+baseurl}/applications/{application%2Did}/federatedIdentityCredentials(name=\'{name}\'){?%24expand,%24select}', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
        return request_info
