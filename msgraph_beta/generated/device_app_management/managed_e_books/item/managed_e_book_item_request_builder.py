...
class ManagedEBookItemRequestBuilder(BaseRequestBuilder):
    ...
    def to_delete_request_information(self,request_configuration: Optional[RequestConfiguration] = None) -> RequestInformation:
        ...
        """
        request_info = RequestInformation(Method.DELETE, '{+baseurl}/deviceAppManagement/managedEBooks/{managedEBook%2Did}', self.path_parameters)
    ...
    def to_patch_request_information(self,body: Optional[ManagedEBook] = None, request_configuration: Optional[RequestConfiguration] = None) -> RequestInformation:
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.PATCH, '{+baseurl}/deviceAppManagement/managedEBooks/{managedEBook%2Did}', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
        request_info.set_content_from_parsable(self.request_adapter, "application/json", body)
        return request_info
    
    def with_url(self,raw_url: Optional[str] = None) -> ManagedEBookItemRequestBuilder:
        """
