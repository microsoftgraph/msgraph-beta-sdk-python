...
class ContentVersionsRequestBuilder(BaseRequestBuilder):
    ...
    def to_post_request_information(self,body: Optional[MobileAppContent] = None, request_configuration: Optional[RequestConfiguration] = None) -> RequestInformation:
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.POST, '{+baseurl}/deviceAppManagement/mobileApps/{mobileApp%2Did}/graph.managedMobileLobApp/contentVersions', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
