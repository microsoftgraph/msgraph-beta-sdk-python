...
add_key_post_request_body = lazy_import('msgraph.generated.applications.item.microsoft_graph_add_key.add_key_post_request_body')
...
class MicrosoftGraphAddKeyRequestBuilder():
    ...
    async def post(self,body: Optional[add_key_post_request_body.AddKeyPostRequestBody] = None, request_configuration: Optional[MicrosoftGraphAddKeyRequestBuilderPostRequestConfiguration] = None) -> Optional[key_credential.KeyCredential]:
        """
    ...
    def to_post_request_information(self,body: Optional[add_key_post_request_body.AddKeyPostRequestBody] = None, request_configuration: Optional[MicrosoftGraphAddKeyRequestBuilderPostRequestConfiguration] = None) -> RequestInformation:
        """
        Add a key credential to an application. This method, along with removeKey, can be used by an application to automate rolling its expiring keys. As part of the request validation for this method, a proof of possession of an existing key is verified before the action can be performed.  Applications that don’t have any existing valid certificates (no certificates have been added yet, or all certificates have expired), won’t be able to use this service action. You can use the Update application operation to perform an update instead.
        Args:
            body: The request body
