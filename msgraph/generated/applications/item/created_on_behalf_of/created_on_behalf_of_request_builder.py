...
class CreatedOnBehalfOfRequestBuilder():
    """
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Optional[Union[Dict[str, Any], str]] = None) -> None:
        ...
        self.url_template: str = "{+baseurl}/applications/{application%2Did}/createdOnBehalfOf{?%24select,%24expand}"

        url_tpl_params = get_path_parameters(path_parameters)
        self.path_parameters = url_tpl_params
        self.request_adapter = request_adapter
    
    def create_get_request_information(self,request_configuration: Optional[CreatedOnBehalfOfRequestBuilderGetRequestConfiguration] = None) -> RequestInformation:
        """
    ...
    async def get(self,request_configuration: Optional[CreatedOnBehalfOfRequestBuilderGetRequestConfiguration] = None, response_handler: Optional[ResponseHandler] = None) -> Optional[directory_object.DirectoryObject]:
        """
        Supports $filter (/$count eq 0, /$count ne 0). Read-only.
        Args:
            requestConfiguration: Configuration for the request such as headers, query parameters, and middleware options.
