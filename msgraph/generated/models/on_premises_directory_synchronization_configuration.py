...
class OnPremisesDirectorySynchronizationConfiguration(AdditionalDataHolder, Parsable):
    ...
    current_export_data: Optional[on_premises_current_export_data.OnPremisesCurrentExportData] = None
    # Interval of time that the customer requested the sync client waits between sync cycles.
    customer_requested_synchronization_interval: Optional[timedelta] = None
    ...
    synchronization_interval: Optional[timedelta] = None
    # Configuration to control how cloud created or owned objects are synchronized back to the on-premises directory.
    writeback_configuration: Optional[on_premises_writeback_configuration.OnPremisesWritebackConfiguration] = None
