...
microsoft_graph_application_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_application.microsoft_graph_application_request_builder')
microsoft_graph_device_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_device.microsoft_graph_device_request_builder')
microsoft_graph_group_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_group.microsoft_graph_group_request_builder')
microsoft_graph_org_contact_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_org_contact.microsoft_graph_org_contact_request_builder')
microsoft_graph_service_principal_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_service_principal.microsoft_graph_service_principal_request_builder')
microsoft_graph_user_request_builder = lazy_import('msgraph.generated.groups.item.owners.microsoft_graph_user.microsoft_graph_user_request_builder')
...
class OwnersRequestBuilder():
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Optional[Union[Dict[str, Any], str]] = None) -> None:
        ...
        self.url_template: str = "{+baseurl}/groups/{group%2Did}/owners{?%24top,%24skip,%24search,%24filter,%24count,%24orderby,%24select,%24expand}"

        url_tpl_params = get_path_parameters(path_parameters)
        self.path_parameters = url_tpl_params
        self.request_adapter = request_adapter
    
    async def get(self,request_configuration: Optional[OwnersRequestBuilderGetRequestConfiguration] = None) -> Optional[directory_object_collection_response.DirectoryObjectCollectionResponse]:
        """
    ...
    def to_get_request_information(self,request_configuration: Optional[OwnersRequestBuilderGetRequestConfiguration] = None) -> RequestInformation:
        """
        The owners of the group who can be users or service principals. Nullable. If this property is not specified when creating a Microsoft 365 group, the calling user is automatically assigned as the group owner.  Supports $filter (/$count eq 0, /$count ne 0, /$count eq 1, /$count ne 1); Supports $expand including nested $select. For example, /groups?$filter=startsWith(displayName,'Role')&$select=id,displayName&$expand=owners($select=id,userPrincipalName,displayName).
        Args:
            requestConfiguration: Configuration for the request such as headers, query parameters, and middleware options.
