...
class UpdatableAssetItemRequestBuilder(BaseRequestBuilder):
    ...
    async def delete(self,request_configuration: Optional[UpdatableAssetItemRequestBuilderDeleteRequestConfiguration] = None) -> None:
        """
        Delete an azureADDevice object. When a Microsoft Entra device is deleted, it is unregistered from the deployment service and automatically unenrolled from management by the service for all update categories, as well as removed from every deploymentAudience and updatableAssetGroup. This API is available in the following national cloud deployments.
